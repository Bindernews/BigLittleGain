...
def  main():
  ...
  installer = "\installer\BigLittleGain Installer.exe"
   
  if demo:
    installer = "\installer\BigLittleGain Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BigLittleGain manual.pdf" 
  ]
  ...
  ZIPNAME = "BigLittleGain-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BigLittleGain-v" + FULLVERSIONSTR + "-win-demo.zip"
